...
import mensageria.util.StringFormatter;
...
public class Solution {
  ...
  public static String processMessages(List<String> messages) {
    ...
    List<AbstractActionModel> actions = messages.stream()
            .map(message -> EventSchema.ofMessage(message).toDTO(message))
            .collect(Collectors.toList());
    actions.sort(Comparator.comparing(AbstractActionModel::getTimestamp));
    actions.forEach(AbstractActionModel::process);

    ValidatorFactory validatorFactory = Validation.buildDefaultValidatorFactory();
    Validator validator = validatorFactory.getValidator();

    List<LoanProposal> validLoanProposals = LoanProposalRepository.findAll().stream()
              .filter(loanProposal -> validator.validate(loanProposal).isEmpty())
              .collect(Collectors.toList());

    LoanProposalRepository.clear();

    return StringFormatter.format(validLoanProposals.stream()
            .map(LoanProposal::getId)
            .map(UUID::toString)
            .collect(Collectors.toList()));
  }
}
